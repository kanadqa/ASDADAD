...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem("Менеджер: Выгрузить данные с БД", "managerRedrawMySheet")
    .addSeparator()
...
function protectIdColumns_() {
  ...
  sheetsToProtect.forEach(sheetName => {
    ...
    const headers = sh.getRange(headerRow_(), 1, 1, lastCol).getValues()[0];
    ...
    if (strict) strictLocks++;
    else warningLocks++;
  });

  SpreadsheetApp.getUi().alert(
    `Готово ✅ Защита ID обновлена.
Строгая защита: ${strictLocks}
Режим предупреждения: ${warningLocks}`
  );
}
